...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function toggleUpdateComment(commentId) {
    ...
    commentPicElement.addEventListener('change', (event) => {
      ...
      if (files) {
        ...
        fetch('https://api.cloudinary.com/v1_1/dhgs8thzx/image/upload', {
          ...
          .catch((error) => {
            ...
            errorMsgElement.innerText = "Error uploading the image. Please try again.";
          });
      }
    });
    ...
    updateCommentForm.addEventListener("submit", (e) => {
      ...
      const updatedCommentPic = uploadImageUrl;
  ...
  function toggleUpdatePost(postId) {
    ...
    postPicElement.addEventListener('change', (event) => {
      const target = event.target;
      const files = target.files;
      ...
      if (files) {
        ...
        // Display a loading spinner or progress bar here if you want
        ...
        fetch('https://api.cloudinary.com/v1_1/dhgs8thzx/image/upload', {
          ...
          .then((res) => {
            if (!res.ok) {
              throw new Error('Image upload failed'); // Handle non-OK responses
            }
            return res.json();
          })
          .then((res) => {
            uploadImageUrl = res.url;
            // You can hide the loading spinner or progress bar here if you displayed one
          })
          .catch((error) => {
            console.error('Image upload error:', error);
            errorMsgElement.innerText = "Error uploading the image. Please try again.";
    ...
    updatePost.addEventListener("submit", (e) => {
      ...
      const updatedpostPic = uploadImageUrl;
