describe('Authentication Forms', () => {
    ...
    it('Should Log in and like a post', () => {
        ...
        cy.window().its('localStorage').invoke('getItem', 'userId').then((userId) => {
          // userId will contain the user's ID stored in local storage
          // Assuming that you have this ID, you can now locate and click the "like" button
        //   cy.get('.numLikes').first().click();
        cy.get('.post:nth-child(2) .numLikes').click();
      
          // You can add assertions or further actions here to confirm that the like was successful
          // For example, you can check for a visual change indicating the post has been liked
          // or check if the number of likes has increased.
        });
      });
