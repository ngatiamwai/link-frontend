            ...
            if (pool.connected) {
                ...
                const user = (
                    await pool
                        .request()
                        .input("email", email)
                        .query("SELECT id, username FROM userTable WHERE email = @email")
                ).recordset[0];
                    ...
                    async (err, html) => {
                        ...
                        try {
                            ...
                            await pool
                                ...
                                .query(
                                    "UPDATE userTable SET resetToken = @resetToken, resetTokenExpiry = @resetTokenExpiry WHERE id = @id"
                                );
                        } catch (error) {
    ...
    updatePassword: async (req, res) => {
        try {
            ...
            const user = await pool.request()
                .input("resetToken", resetToken)
                .query("SELECT id, resetTokenExpiry FROM userTable WHERE resetToken = @resetToken");
            ...
            await pool.request()
                ...
                .query(
                    "UPDATE userTable SET password = @newPassword, resetToken = NULL, resetTokenExpiry = NULL WHERE resetToken = @resetToken"
                );
